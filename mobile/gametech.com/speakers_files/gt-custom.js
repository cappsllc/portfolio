...
function closebio() {
	...
	$(document).ready(function() {
		...
		if (supports_html5_storage() && supports_json()) {
			//alert('supports local storage and json');
			...
			$(".addToFavoritesDiv a").live('click', function(event) {
				var id=$(this).data("artid");
				var idpage = id+ 'addtofav.html';
				var aPage = idpage.toString();
				//alert(sPage);
				$.mobile.changePage(aPage, {role:"dialog",data:{"id":id}});
			});

			//When clicking the link in details pages to add to fav
			$(".removeFromFavoritesDiv a").live('click', function(event) {
				var id=$(this).data("artid");
				var idpage = id+ 'removefromfav.html';
				var sPage = idpage.toString();
				//alert(sPage);
				$.mobile.changePage(sPage, {role:"dialog",data:{"id":id}});				
			});

			//When confirming the add to fav
			$('.addToFavoritesButton').live('click', function(event, ui){
				var id=$(this).data("artid");
			...
			$('.removeFromFavoritesButton').live('click', function(event, ui){
				var id=$(this).data("artid");
				var label=$(this).data("artname");
			...
			$('#homePage').live('pagebeforeshow', function(event, ui){
				...
				if (!$.isEmptyObject(favs)) {
					//alert('schedule not empty');
					if ($favoritesList.size() == 0) {
						$favoritesList = $('<ul id="favoritesList" data-inset="true" data-theme="d" data-divider-theme="a"></ul>');
						
						var s = "<li data-role=\"list-divider\">Favorites</li>";
						for (var key in favs) {
							s+= "<li><a href=\"/sessions/"+key+".html\">"+favs[key]+"</a></li>";
						}
						$favoritesList.append(s);
						$("#homePageContent").append($favoritesList);	
						$('ul#favoritesList>li').tsort();				
			...
			function getStorage(){
				...
				return window.JSON.parse(current);
				alert(current);
			}
			...
			function saveStorage(data){
				...
				//console.dir(data);
				window.localStorage["favorites"] = window.JSON.stringify(data);
